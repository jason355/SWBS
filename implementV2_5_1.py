...
class Bot():
    ...
    # 回覆樣板
    def reply_cancel(self, event,user_id, text):
        message = TemplateSendMessage(
            ...
            for group in number_groups:
                if len(group) == 1:   # 判斷為年級或班級
                    ...
                    if group in class_list:
                        if int(group[0:1]) < 4: 
                            if group[0] not in number_groups and str(int(group[0:2]) -9) not in number_groups:
    ...
    def handle_Ss2(self, event, user_id, text):
        ...
        self.sendSettingConfirm(event,user_id, reply)


    # 個人資訊確認按鈕
    def sendSettingConfirm(self, event,user_id, text):
        try:
            message = TemplateSendMessage(
                alt_text='Button template',
                template=ButtonsTemplate(
