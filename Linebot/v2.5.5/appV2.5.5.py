import re
import implementV2_5_5 as f
from implementV2_5_5 import Teacher, Bot
import databaseV2_5_5 as db
...
忘記如何使用？在選單點選HELP按鈕查看說明。'''
    ...
    elif backdata.get("action") == "@DD":
        reply_message = "尚未開發"
        Manager.users[user_id].status = "Fs"
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text=reply_message))

    elif backdata.get("action") == "@MD":
        if Manager.users[user_id].status == "Hs":
            Manager.modifyDataButtons(event)
