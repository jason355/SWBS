...
async def handle_message(ws):
    ...
    while True:
        ...
        try:
            ...
            print(f"WebSocket connection closed OK, class = {client_data[ws]}")
            await ws.close()
            break
...
async def send_message_to_user(message, id, dest):
    ...
    for ws, cls_infor in client_data.items():
        ...
        if int(cls_infor["classCode"]) == int(dest):
            ...
            return await send(ws, data, id, "S1")


async def send(ws, message, id, header):
                    ...
                    for datas in check:
                        ...
                        for key in client_data:
                            if int(client_data[key]["classCode"]) == int(dest):
                                # send message
                                sent = await send_message_to_user(record, str(datas.id), dest)
